...
means, adding labels, and performing other preprocessing steps. It uses
the `pandas` and `numpy` libraries for data manipulation and the
...
Classes:
    DataPreprocessor:
        ...
        and performing other preprocessing steps.

Example:
    To prepare data for RNN training, instantiate the RNNDataPrep class
    and call the get_rnn_data method:

    >>> rnn_data_prep = RNNDataPrep()
    >>> X_train, Y_train, X_test, Y_test = rnn_data_prep.get_rnn_data(
    >>>     sequence_length=5, split_ratio=0.7
    >>> )

Note:
    The module assumes the presence of a 'data/interim/' directory
    containing the raw data in pickle format and a
    'data/processed/rnn_input/' directory for saving processed data. The
    class is designed to work with time-series or sequence data, which
    is typical for RNN models.
"""
...
class RNNDataPrep:
    ...
    Attributes:
        ...
        train_test_data_par_dir (str): The parent directory for saving
            the train and test datasets.
        save_train_test (bool): If True, the train and test datasets are
            saved.
        ...
        train_test_data_dir (Path): The path to the train and test
        datasets. raw_data_id (str): The hash value of the raw data.
        preprocessor (DataPreprocessor): The DataPreprocessor object for
            preprocessing the raw data.
        feature_engineer (FeatureEngineer): The FeatureEngineer object
            for performing feature engineering on the preprocessed data.
        test_indices (np.ndarray): The indices of the target values for
            the test sequences.

    Methods:
        get_rnn_data(
            load_processed, load_train_test, sequence_length,
            split_ratio, save_data, save_train_test
        ): Provides an interface for either loading preprocessed
            data or preprocessing raw data, performing feature
            engineering, preparing sequences and train-test splits, and
            saving the processed data and train-test splits.
        load_and_preprocess_data(save_data): Loads the raw data and
            performs preprocessing and feature engineering steps.
        prepare_rnn_data(
            df, sequence_length, split_ratio, rand_oversample ,
            save_train_test
        ...
        _create_seqs(data, sequence_length, index_start): Creates
            sequences of length `sequence_length` from the input `data`.
        _perform_random_oversampling(X_train, Y_train): Performs random
            oversampling to balance the class distribution.
        _load_train_test_data(): Loads the train and test datasets for
            the RNN model from 4 .pkl files.
        _save_train_test_data(X_train, Y_train, X_test, Y_test): Saves
            the train and test datasets for the RNN model as .pkl files.
    """

    def __init__(
        self,
        pickle_path: str = "data/interim/ff-mw.pkl",
        train_test_data_par_dir: str = "data/processed/rnn_input/",
        ...
        Args:
            pickle_path (str, optional): The path to the raw data in
                pickle format. Defaults to "data/interim/ff-mw.pkl".
            train_test_data_par_dir (str, optional): The parent
                directory for saving the train and test datasets.
                Defaults to "data/processed/rnn_input/".
            save_train_test (bool, optional): If True, the train and
                test datasets are saved. Defaults to False.
            save_data (bool, optional): If True, the processed data is
    ...
    def get_rnn_data(
        ...
        """
        Provides an interface for either loading preprocessed data or
        preprocessing raw data, performing feature engineering,
        preparing sequences and train-test splits, and saving the
        processed data and train-test splits.

        Args:
            load_processed (bool, optional): If True, preprocessed data
                is loaded. Defaults to False.
            load_train_test (bool, optional): If True, train and test
                datasets are loaded. Defaults to False.
            sequence_length (int, optional): The length of the
                sequences. Defaults to 3.
            split_ratio (float, optional): The ratio of train to test
                sequences. Defaults to 2/3.
            save_data (bool, optional): If True, the processed data is
                saved. Defaults to None.
            save_train_test (bool, optional): If True, the train and
                test datasets are saved. Defaults to None.

        Returns:
            Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]: The
                train and test datasets as NumPy arrays.
        """
        ...
        logger.info(
            "\tThe steps are:- \n\t\tif load_train_test=True, "
            "(1) Load existing train/test data; \n\t\tELSE, (1) Load and "
            "preprocess raw/interim data, (2) Prepare RNN train & test "
            "datasets from the preprocessed data, (3) Save the train & test "
            "datasets (if save_train_test = True)."
        )

        # Set the values of self.save_train_test and self.save_data to
        # the values of save_train_test and save_data if they are not
        # None
        if save_train_test is not False:
            self.save_train_test = save_train_test
        ...
        if load_train_test:
            logger.info(
                "Load preprocessed train & test datasets in RNNDataPrep -> "
                "get_rnn_data() ..."
            )
            ...
            logger.info(
                "Train & test datasets loaded successfully in RNNDataPrep "
                "-> get_rnn_data()."
            )
        elif load_processed:
            logger.info(
            ...
            logger.info(
                "Processed dataframe loaded successfully in RNNDataPrep -> "
                "get_rnn_data()."
            )
            ...
            logger.info(
                "Prepare RNN train & test datasets from raw/interim data "
                "in RNNDataPrep -> get_rnn_data() ..."
            )
            ...
            logger.info(
                "Load raw/interim data to be preprocessed/prepared in "
                "RNNDataPrep -> get_rnn_data() ..."
            )
            self.df = self.load_and_preprocess_data()
            logger.info(
                "Raw/interim data loaded successfully in RNNDataPrep -> "
                "get_rnn_data()."
            )
            logger.info(
                "Prepare RNN train & test datasets from raw/interim data "
            ...
            logger.info(
                "Train & test datasets prepared successfully in "
                "RNNDataPrep -> get_rnn_data()."
            )

        # Logging messages to indicate the end of the function
        logger.info(
            "RNN train & test datasets retrieved successfully in "
            "RNNDataPrep -> get_rnn_data().\n\n"
        )
    ...
    def load_and_preprocess_data(
        ...
        """
        Loads the raw data and performs preprocessing and feature
        engineering steps.
        ...
        # Logging messages to indicate the start of the function
        logger.info(
            "\nLoading & preprocessing raw/interim data to be prepared for "
            "RNN in RNNDataPrep -> load_and_preprocess_data..."
        )
        ...
        logger.info(
            "Performing feature engineering steps on preprocessed raw/interim "
            "data..."
        )
        ...
        # Logging messages to indicate the end of the function
        logger.info(
            "Data loading, preprocessing, and feature engineering completed in"
            " RNNDataPrep -> load_and_preprocess_data.\n"
        )
        return self.df
    ...
    def prepare_rnn_data(
        ...
        Args:
            df (pd.DataFrame): The DataFrame to prepare the train and
                test datasets from.
            sequence_length (int, optional): The length of the
                sequences. Defaults to 3.
            split_ratio (float, optional): The ratio of train to test
                sequences. Defaults to 2/3.
            rand_oversample (bool, optional): If True, random
                oversampling is performed to balance the class
        ...
        # Logging messages to indicate the start of the function
        logger.info(
            "\nPreparing sequences and train-test splits for RNN in "
            "RNNDataPrep -> prepare_rnn_data..."
        )

        # Set the value of self.save_train_test to the value of
        # save_train_test if save_data is not None
        if save_train_test is not False:
            self.save_train_test = save_train_test
        ...
        if self.save_train_test:
            ...
            self.rnn_data_path = self._save_train_test_data(
                X_train, Y_train, X_test, Y_test
            )

        # Logging messages to indicate the end of the function
        logger.info(
            "Sequences and train-test splits prepared successfully in "
            "RNNDataPrep -> prepare_rnn_data.\n"
        )
        return X_train, Y_train, X_test, Y_test

    def _prep_train_test_seqs(
        self, df: pd.DataFrame, sequence_length: int, split_ratio: float
        ...
        Args:
            df (pd.DataFrame): The DataFrame to prepare the train and
            test datasets from. sequence_length (int): The length of the
                sequences.
        ...
        """
        logger.info(
            "\nStarting to prepare train and test sequences in RNNDataPrep -> "
        ...
        # Convert DataFrame to NumPy array after dropping unnecessary
        # columns
        ...
        min_indices = (
            df.groupby("file").apply(lambda x: x.index.min()).to_dict()
        )
        ...
        for i, (file, file_length) in enumerate(
            ...
            # Calculate the end indices for the training and testing
            # data
        ...
        logger.info(
            f"\nPrepared {len(X_train)} training sequences and {len(X_test)} "
            "testing sequences."
        )
        logger.info(
            "Train and test sequence preparation completed in RNNDataPrep -> "
            "_prep_train_test_seqs.\n"
        )
    ...
    def _create_seqs(
        ...
        Args:
            ...
            index_start (int, optional): The starting index for the
                sequences. Defaults to 0.

        Returns:
            Tuple[np.ndarray, np.ndarray]: The sequences and target
    ...
    def _perform_random_oversampling(self, X_train, Y_train):
        ...
        # Logging messages comparing the original and resampled dataset
        # shapes for both X_train and Y_train with appropriate
        # formatting and spacing in the printed output (using f-strings
        # and :> formatting)
        logger.info(
    ...
    def _load_train_test_data(
        ...
        """
        Loads the train and test datasets for the RNN model from 4 .pkl
        files.
        ...
        try:
            ...
            X_train_file = (
                Path(self.train_test_data_dir) / "X_train.pkl"
            )  # type: ignore
            Y_train_file = (
                Path(self.train_test_data_dir) / "Y_train.pkl"
            )  # type: ignore
            X_test_file = (
                Path(self.train_test_data_dir) / "X_test.pkl"
            )  # type: ignore
            Y_test_file = (
                Path(self.train_test_data_dir) / "Y_test.pkl"
            )  # type: ignore
    ...
    def _save_train_test_data(
        ...
        """
        Saves the train and test datasets for the RNN model as .pkl
        files.
