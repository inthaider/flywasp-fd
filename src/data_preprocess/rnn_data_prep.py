"""
This module contains the `DataPreprocessor` class for preprocessing a Pandas
DataFrame.

The `DataPreprocessor` class includes methods for loading data, saving
processed data, dropping columns, rearranging columns, calculating means,
adding labels, and performing other preprocessing steps. It uses the `pandas`
and `numpy` libraries for data manipulation and the `logging` library for
logging.

Classes:
    DataPreprocessor:
        A class for preprocessing a Pandas DataFrame. It includes methods for
        loading data, saving processed data, dropping columns, rearranging
        columns, calculating means, adding labels, and performing other
        preprocessing steps.

Example:
    To prepare data for RNN training, instantiate the RNNDataPrep class and
    call the get_rnn_data method:
...
Note:
    The module assumes the presence of a 'data/interim/' directory containing
    the raw data in pickle format and a 'data/processed/rnn_input/' directory
    for saving processed data. The class is designed to work with time-series
    or sequence data, which is typical for RNN models.
"""
...
class RNNDataPrep:
    ...
    Attributes:
        ...
        train_test_data_par_dir (str): The parent directory for saving the
        ...
        feature_engineer (FeatureEngineer): The FeatureEngineer object for
            performing feature engineering on the preprocessed data.
        test_indices (np.ndarray): The indices of the target values for the
            test sequences.

    Methods:
        get_rnn_data(
                load_processed, load_train_test, sequence_length, split_ratio,
                save_data, save_train_test
            ): Provides an interface for either loading preprocessed data or
            preprocessing raw data, performing feature engineering, preparing
            sequences and train-test splits, and saving the processed data and
            train-test splits.
        load_and_preprocess_data(save_data): Loads the raw data and performs
            preprocessing and feature engineering steps.
        prepare_rnn_data(
                df, sequence_length, split_ratio, rand_oversample
                , save_train_test
            ): Prepares the train and test datasets for the RNN model.
        _prep_train_test_seqs(df, sequence_length, split_ratio): Prepares
            training and testing sequences for the RNN model.
        _create_seqs(data, sequence_length, index_start): Creates sequences of
            length `sequence_length` from the input `data`.
        _perform_random_oversampling(X_train, Y_train): Performs random
            oversampling to balance the class distribution.
        _load_train_test_data(): Loads the train and test datasets for the RNN
            model from 4 .pkl files.
        _save_train_test_data(X_train, Y_train, X_test, Y_test): Saves the
            train and test datasets for the RNN model as .pkl files.
    """
