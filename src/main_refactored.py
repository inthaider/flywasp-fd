...
# Move logging setup to the top-level
logging.basicConfig(level=logging.INFO)


def consistent_hashing(input_data):
    return hashlib.md5(str(input_data).encode('utf-8')).hexdigest()


def preprocessing_steps(preprocessor, pickle_path):
    ...
    preprocessor.specific_rearrange("F2Wdis_rate", "F2Wdis")  # Rearrange the column names
...
def feature_engineering_steps(feature_engineer):
...
def main():
    ...
    preprocessing_steps(preprocessor, pickle_path)

    # Perform feature engineering steps
    feature_engineer = FeatureEngineer(df=preprocessor.df)
    feature_engineering_steps(feature_engineer)
    ...
    X_train, Y_train, X_test, Y_test = prepare_train_test_sequences(feature_engineer.df)
