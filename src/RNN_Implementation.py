...
def create_sequences(data, sequence_length=10, start_index=0, skip=0):
    ...
    for i in range(n):
        ...
        target = data[
            end_i + 1, -1
        ]  # The target is the immediate next frame after the sequence
...
directory = (
    r"D:\Fly-Wasp\Data\Pickle Objects\Wild-Type"
    r"\Train Test Data For ff-mw RNN"
)
...
class RNN(nn.Module):
    def __init__(
        self, input_size, hidden_size, output_size, intermediate_size
    ):
        ...
        self.rnn = nn.RNN(
            input_size, hidden_size, batch_first=True, nonlinearity="relu"
        )
...
for epoch in range(num_epochs):
    ...
    test_loss, test_acc, test_f1, test_pr_auc = evaluate(
    ...
    print(
        ...
        f"Test F1: {test_f1:.4f}, Test PR AUC: {test_pr_auc:.4f}"
    )
...
merged_df = pd.DataFrame(
    {"index": test_indices, "y_test": all_labels, "y_pred": all_preds_probs}
)
...
def flag_frames(group):
    ...
    start_indices = group.index[
        group["start_walk"] == 1
    ]  # Indices where 'start_frame' equals 'Frame'
    for idx in start_indices:
        # Flag frames that are within a 40-frame value distance before
        # the current frame
        mask = (group["Frame"] >= group.loc[idx, "Frame"] - 200) & (
...
# Plot of predictions within 200 frames (5 seconds). Assuming mean_df is
# your DataFrame and it has columns 'delta_frames' and 'y_pred'
plt.figure(
    figsize=(10, 6)
)  # Create a new figure with a specific size (optional)
plt.plot(
    mean_df["delta_frames"], mean_df["y_pred"]
)  # Plot 'y_pred' against 'delta_frames'
